...
汰。可是互联网在有的地区并不互联，需要我们使用魔法来打通连接，本篇教程就是魔法咒语，学会吟诵咒语则你离现代又近了一步。

墙墙了我们的眼见，我们以为自己的产品已经很不错了，可简单的用了一些国外的软件后，发现它们用户体验更好，设计得更简洁好看酷炫更符合逻辑和自然交互，而
且还没那么多诱导广告和金融借贷，没有那么多实名制和没必要的审核。

墙不仅墙了我们的眼界，还墙了我们的认知（当然眼界影响认知），所以突破信息茧房还是挺重要的！

墙外并非没有假消息，而是真假消息并存，可能假消息远比真消息还多，
需要我们自己去分辨，但是它的特点是丰富是多样性，真相通常不会被人为的压制，不会存在这么多的封锁、隐瞒、篡改、编造、夸张、变形、扭曲、立场、意识形态、政治正确、情绪化、黑箱黑洞、删帖、报喜不报忧、息事宁人，（典型示例即三年疫情期间的信息状况），信息源头不会被截断，这点很重要，因为你可以溯源，你直接到源头去查看信息，了解事情的原委，避免被 hack，避免消息被别有用心的二道贩子裁剪和篡改，拨云见日破除谣言直达真相。

此教程笔者参考了 [haoel.github.io][haoel]，步骤皆为笔者亲手实践过，适用于国内情况，可能稍显复杂，但相对安全，毕竟安全第一，谁也不想 “大 Boss 正在看着你”，如果读者觉得麻烦可参考笔者的另一篇博文 [科学上网之 Outline 方案](../11/科学上网之Outline方案.html)。

我们翻墙后，不要谈论政治和敏感话题，翻墙的目的是了解真相和学习知识、文化、技能、技术、英语和看剧（奈飞、迪士尼+、Apple TV+），用于提升自身水平以便更好的工作生活（即挣更多的 💰）。拿工程技术来说，尤其软件开发，应该用英文关键字在谷歌上搜索资料（中英文搜出的结果不一样，可用维基百科验证），用好 GitHub 和 Stack Overflow。
...
- 点击所有产品和服务中的 DNS 。
- 将 GoDaddy 的名称服务器替换为 Cloudflare 的名称服务器并保存。名称服务器地址由 Cloudflare 页面给出，类似以下地址：
...
- 下载并安装 [ClashX](https://github.com/paradiseduo/ClashXR/releases)。
...
- 下载并安装 [Clash for Windows](https://github.com/Z-Siqi/Clash-for-Windows_Chinese/releases)。
...
- 下载并安装 [Clash for Android](https://github.com/ccg2018/ClashA/releases)。
