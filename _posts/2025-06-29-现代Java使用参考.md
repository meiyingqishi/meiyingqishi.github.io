...
- [正文](#正文)
  - [1. Java 8](#1-java-8)
    - [1.1 Lambda 表达式](#11-lambda-表达式)
    - [1.2 Stream API](#12-stream-api)
    - [1.3 Optional 类](#13-optional-类)
    - [1.4 接口的默认方法和静态方法](#14-接口的默认方法和静态方法)
    - [1.5 新的日期时间 API](#15-新的日期时间-api)
  - [2. Java 9](#2-java-9)
    - [2.1 接口的私有方法](#21-接口的私有方法)
    - [2.2 模块系统](#22-模块系统)
    - [2.3 集合工厂方法](#23-集合工厂方法)
    - [2.4 JShell：交互式 Java](#24-jshell交互式-java)
  - [3. Java 10](#3-java-10)
    - [3.1 局部变量类型推断 var](#31-局部变量类型推断-var)
  - [4. Java 11](#4-java-11)
    - [4.1 Lambda 参数中使用 var](#41-lambda-参数中使用-var)
    - [4.2 标准化的 HTTP 客户端](#42-标准化的-http-客户端)
    - [4.3 字符串增强方法](#43-字符串增强方法)
  - [5. Java 14](#5-java-14)
    - [5.1 Switch 表达式](#51-switch-表达式)
  - [6. Java 15](#6-java-15)
    - [6.1 文本块](#61-文本块)
    - [6.2 String.formatted()](#62-stringformatted)
  - [7. Java 16](#7-java-16)
    - [7.1 Stream.toList()](#71-streamtolist)
    - [7.2 Record 类](#72-record-类)
    - [7.3 instanceof 的模式匹配](#73-instanceof-的模式匹配)
  - [8. Java 17](#8-java-17)
    - [8.1 密封类和接口](#81-密封类和接口)
  - [9. Java 18](#9-java-18)
    - [9.1 UTF-8 默认字符集](#91-utf-8-默认字符集)
    - [9.2 简单 Web 服务器](#92-简单-web-服务器)
  - [10. Java 21](#10-java-21)
    - [10.1 switch 的模式匹配](#101-switch-的模式匹配)
    - [10.2 Record 模式](#102-record-模式)
    - [10.3 虚拟线程](#103-虚拟线程)
    - [10.4 有序集合](#104-有序集合)
  - [11. Java 22](#11-java-22)
    - [11.1 未命名模式和变量](#111-未命名模式和变量)
  - [12. Java 24](#12-java-24)
    - [12.1 流收集器](#121-流收集器)
  - [13. Java 25](#13-java-25)
    - [13.1 紧凑源文件和实例主方法](#131-紧凑源文件和实例主方法)
    - [13.2 作用域值](#132-作用域值)
    - [13.3 新的 IO 类](#133-新的-io-类)
    - [13.4 模块导入声明](#134-模块导入声明)
    - [13.5 灵活的构造函数体](#135-灵活的构造函数体)
    - [13.6 密钥派生函数 API](#136-密钥派生函数-api)
  - [14. Java 26](#14-java-26)
    - [14.1 HTTP/3 支持](#141-http3-支持)
...
在兼容性这方面，Golang 幸运得多，它工具链中的工具 gofix（`go fix ./...`）能安全稳定高效的批量更新代码库，
...
**描述**：在 Java 25 中正式标准化为**紧凑源文件和实例主方法**（此前预览阶段被称为“隐式声明的类和实例主方法”）。
这彻底改变了 Java 的入门体验。开发者现在可以编写不包含在显式 class 声明中的简单程序，
并且 main 方法不再需要是 public static 修饰的。这使得编写第一个 Java 程序变得前所未有的简单 and 直观。
...
**描述**：引入了 `import module M;` 语法。允许在普通的 `.java` 源码文件中直接导入整个模块（如 `java.base`）所导出的所有包，
而不需要逐个导入具体的包或类。这极大简化了引入标准库或大型库的样板代码。
...
**描述**：在此特性之前，构造函数中的 this(...) 或 super(...) 调用必须是第一条语句。
...
**描述**：引入了一个新的、标准化的 API (javax.crypto.KDF)，用于密钥派生函数。
...
**描述**: 为标准的 `java.net.http.HttpClient` API 增加了对 HTTP/3 协议的原生支持。
HTTP/3 基于 QUIC（UDP）协议，相较于基于 TCP 的 HTTP/1.1 和 HTTP/2，具有更低的连接建立延迟和更好的弱网表现。
使用方式非常简单，只需在构建 HttpClient 或 HttpRequest 时指定 `HttpClient.Version.HTTP_3` 即可。
如果服务器不支持 HTTP/3，客户端会自动优雅降级到 HTTP/2 或 HTTP/1.1。
...
从上到下排列，以便写代码时抓住重点，先写先验证核心部分。清单上下分成两大部分，用横线从中分隔，前部分是
...
或者人员字段 createdBy、updatedBy…… 。最好先用 AI 查一下。
